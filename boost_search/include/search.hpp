...
class Search {
   public:
    ...
    void SearchForKeyWord(const std::string& word, std::string* json_str) {
        ...
        std::unordered_map<uint64_t, ns_index::UniqueElem>
            unique_list;  // 去重累加计算
        ...
        for (auto& s : cut_result) {
            ...
            // 可能有多个词对应一个文档，此时就会出现
            // {inelem1，inelem2，inelem1}因此后续查找会找出相同的html
            // all_inverter_list.insert(all_inverter_list.end(), list->begin(),
            // list->end()); //可能会重复
            ...
            for (const auto& e : *list) {
                ...
                value.weight +=
                    e.weight;  // 如果不存在，正常复制；存在则累加权重
                value.word_list.push_back(
                    e.word);  // 不重复则只插入一个，重复则会插入重复的搜索分词
            }
        }
        // 生成去重后的倒排
        ...
        std::sort(all_inverter_list.begin(), all_inverter_list.end(),
                  [](ns_index::UniqueElem& e1, ns_index::UniqueElem& e2) {
                      return e1.weight > e2.weight;
                  });

        // 没去重
        // std::sort(all_inverter_list.begin(), all_inverter_list.end(),
        // [](const ns_index::InvertedElem& e1, const ns_index::InvertedElem&
        // e2){
        //     return e1.weight > e2.weight;
        // });
   ...
   private:
    ...
    std::string GetDesc(const std::string& content, const std::string& word) {
        ...
        auto iter =
            std::search(content.begin(), content.end(), word.begin(),
                        word.end(), [](const char& s1, const char& s2) {
                            return (std::tolower(s1) == std::tolower(s2));
                        });

        std::size_t pos =
            std::distance(content.begin(), iter);  // 计算两个迭代器的距离
        if (pos > 50) {
            // pos前面有50字符，往前截50
            begin = pos - 50;
        }
        ...
        // std::cout << " begin:" << begin << " end:" << end << std::endl; for
        // debug
